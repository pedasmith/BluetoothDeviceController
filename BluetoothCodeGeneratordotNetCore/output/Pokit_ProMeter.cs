...
namespace BluetoothProtocols
{
    ...
    public partial class Pokit_ProMeter : INotifyPropertyChanged
    {
    ...
    // Link: https://github.com/pcolby/qtpokit
        ...
        public async Task WriteServer_Supported_Features(byte FeatureBitmap0)
        {
            ...
            {
                await WriteCommandAsync(0, "Server_Supported_Features", command, GattWriteOption.WriteWithResponse);
            }
...
[[ERROR: Properties has no macros and no children; parent=Service Changes template=SET+PROPERTY+VALUES]]
        ...
        public async Task WriteClient_Supported_Features(byte FeatureBitmap0)
        {
            ...
            if (command.Length <= MAXBYTES) //TODO: make sure this works
            {
                await WriteCommandAsync(2, "Client_Supported_Features", command, GattWriteOption.WriteWithResponse);
            }
            else for (int i=0; i<command.Length; i+= MAXBYTES)
            {
                // So many calculations and copying just to get a slice
        ...
        public async Task WriteDevice_Name(String Device_Name)
        {
            ...
            if (command.Length <= MAXBYTES) //TODO: make sure this works
            {
                await WriteCommandAsync(4, "Device_Name", command, GattWriteOption.WriteWithResponse);
            }
            else for (int i=0; i<command.Length; i+= MAXBYTES)
            {
                // So many calculations and copying just to get a slice
        ...
        public async Task WriteStatus_Device_Name(String Device_Name)
        {
            ...
            if (command.Length <= MAXBYTES) //TODO: make sure this works
            {
                await WriteCommandAsync(14, "Status_Device_Name", command, GattWriteOption.WriteWithResponse);
            }
            else for (int i=0; i<command.Length; i+= MAXBYTES)
            {
                // So many calculations and copying just to get a slice
        ...
        public async Task WriteStatus_Flash_LED(byte Red, byte Green, byte Blue, byte Beep)
        {
            ...
            if (command.Length <= MAXBYTES) //TODO: make sure this works
            {
                await WriteCommandAsync(15, "Status_Flash_LED", command, GattWriteOption.WriteWithResponse);
            }
            else for (int i=0; i<command.Length; i+= MAXBYTES)
            {
                // So many calculations and copying just to get a slice
        ...
        public async Task WriteStatus_Light_LED(byte Light)
        {
            ...
            if (command.Length <= MAXBYTES) //TODO: make sure this works
            {
                await WriteCommandAsync(16, "Status_Light_LED", command, GattWriteOption.WriteWithResponse);
            }
            else for (int i=0; i<command.Length; i+= MAXBYTES)
            {
                // So many calculations and copying just to get a slice
        ...
        public async Task WriteUnknownStatusValues(byte[] StatusUnknown5)
        {
            ...
            if (command.Length <= MAXBYTES) //TODO: make sure this works
            {
                await WriteCommandAsync(17, "UnknownStatusValues", command, GattWriteOption.WriteWithResponse);
            }
            else for (int i=0; i<command.Length; i+= MAXBYTES)
            {
                // So many calculations and copying just to get a slice
        ...
        public async Task WriteMM_Settings(byte Mode, byte Range, UInt32 Interval)
        {
            ...
            if (command.Length <= MAXBYTES) //TODO: make sure this works
            {
                await WriteCommandAsync(18, "MM_Settings", command, GattWriteOption.WriteWithResponse);
            }
            else for (int i=0; i<command.Length; i+= MAXBYTES)
            {
                // So many calculations and copying just to get a slice
        ...
        public async Task WriteDSO_Settings(byte DsoTriggerType, float DsoTriggerLevel, byte DsoMode, byte DsoRange, UInt32 DsoSamplingWindow, UInt16 DsoNSamples)
        {
            ...
            if (command.Length <= MAXBYTES) //TODO: make sure this works
            {
                await WriteCommandAsync(20, "DSO_Settings", command, GattWriteOption.WriteWithResponse);
            }
            else for (int i=0; i<command.Length; i+= MAXBYTES)
            {
                // So many calculations and copying just to get a slice
        ...
        public async Task WriteDataLogger_Settings(byte DlogCommand, UInt16 DlogReserved1, byte DlogMode, byte DlogRange, UInt16 DlogUpdateInterval, UInt32 DlogTimestamp)
        {
            ...
            if (command.Length <= MAXBYTES) //TODO: make sure this works
            {
                await WriteCommandAsync(23, "DataLogger_Settings", command, GattWriteOption.WriteWithResponse);
            }
            else for (int i=0; i<command.Length; i+= MAXBYTES)
            {
                // So many calculations and copying just to get a slice
        ...
        public async Task WriteCalbrateTemperature(byte[] CalibrateUnknown0)
        {
            ...
            if (command.Length <= MAXBYTES) //TODO: make sure this works
            {
                await WriteCommandAsync(26, "CalbrateTemperature", command, GattWriteOption.WriteWithResponse);
            }
            else for (int i=0; i<command.Length; i+= MAXBYTES)
            {
                // So many calculations and copying just to get a slice
        ...
        public async Task WriteCalibrateUnknown1(byte[] CalibrateUnknown1)
        {
            ...
            if (command.Length <= MAXBYTES) //TODO: make sure this works
            {
                await WriteCommandAsync(27, "CalibrateUnknown1", command, GattWriteOption.WriteWithResponse);
            }
            else for (int i=0; i<command.Length; i+= MAXBYTES)
            {
                // So many calculations and copying just to get a slice
        ...
        public async Task WriteCalibrrateUnknown2(byte[] CalibrrateUnknown2)
        {
            ...
            if (command.Length <= MAXBYTES) //TODO: make sure this works
            {
                await WriteCommandAsync(28, "CalibrrateUnknown2", command, GattWriteOption.WriteWithResponse);
            }
            else for (int i=0; i<command.Length; i+= MAXBYTES)
            {
                // So many calculations and copying just to get a slice
        ...
        public async Task WriteOTA_Control(byte[] Unknown0)
        {
            ...
            if (command.Length <= MAXBYTES) //TODO: make sure this works
            {
                await WriteCommandAsync(29, "OTA_Control", command, GattWriteOption.WriteWithoutResponse);
            }
            else for (int i=0; i<command.Length; i+= MAXBYTES)
            {
                // So many calculations and copying just to get a slice
        ...
        public async Task WriteOTA_Data(byte[] Unknown1)
        {
            ...
            if (command.Length <= MAXBYTES) //TODO: make sure this works
            {
                await WriteCommandAsync(30, "OTA_Data", command, GattWriteOption.WriteWithoutResponse);
            }
            else for (int i=0; i<command.Length; i+= MAXBYTES)
            {
                // So many calculations and copying just to get a slice
