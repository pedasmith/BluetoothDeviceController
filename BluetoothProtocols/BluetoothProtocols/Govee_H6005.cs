...
namespace BluetoothProtocols
{
    ...
    /// This class was automatically generated 2023-01-16::01:56
    /// </summary>
    ...
    // Link: https://github.com/chvolkmann/govee_btled/blob/master/govee_btled/bluetooth_led.py
        ...
        Guid[] CharacteristicGuids = new Guid[] {
            ...
            Guid.Parse("00010203-0405-0607-0809-0a0b0c0d2b10"), // #0 is Rx
            Guid.Parse("00010203-0405-0607-0809-0a0b0c0d2b11"), // #1 is Tx
        ...
        String[] CharacteristicNames = new string[] {
            ...
            "Rx", // #0 is 00010203-0405-0607-0809-0a0b0c0d2b10
            "Tx", // #1 is 00010203-0405-0607-0809-0a0b0c0d2b11
        ...
        public enum CharacteristicsEnum {
            ...
            Service_Changes_Generic_Service_enum = 3,
            PnP_ID_Device_Info_enum = 4,
            Rx_TeCommand_enum = 5,
            Tx_TeCommand_enum = 6,
        ...
        private string _Rx = null;
        private bool _Rx_set = false;
        public string Rx
        {
            get { return _Rx; }
            internal set { if (_Rx_set && value == _Rx) return; _Rx = value; _Rx_set = true; OnPropertyChanged(); }
        }
        ...
        /// <param name="cacheMode">Caching mode. Often for data we want uncached data.</param>
        /// <returns>BCValueList of results; each result is named based on the name in the characteristic string. E.G. U8|Hex|Red will be named Red</returns>
        public async Task<BCBasic.BCValueList> ReadRx(BluetoothCacheMode cacheMode = BluetoothCacheMode.Uncached)
        {
            if (!await EnsureCharacteristicAsync(CharacteristicsEnum.Rx_TeCommand_enum)) return null;
            IBuffer result = await ReadAsync(CharacteristicsEnum.Rx_TeCommand_enum, "Rx", cacheMode);
            if (result == null) return null;

            var datameaning = "BYTES|HEX|Rx";
            var parseResult = BluetoothDeviceController.BleEditor.ValueParser.Parse(result, datameaning);
            Rx = parseResult.ValueList.GetValue("Rx").AsString;
        ...
        // Returns a string with the status; starts with OK for good status.
        /// <summary>
        /// Event for notifications; RxEvent += _my function_
        /// </summary>
        public event BluetoothDataEvent RxEvent = null;
        /// <summary>
        /// We only want to set the internal callback once, and never need to remove it.
        /// </summary>

        private bool NotifyRx_ValueChanged_Set = false;
        ...
        public async Task<bool> NotifyRxAsync(GattClientCharacteristicConfigurationDescriptorValue notifyType = GattClientCharacteristicConfigurationDescriptorValue.Notify)
        {
            if (!await EnsureCharacteristicAsync(CharacteristicsEnum.Rx_TeCommand_enum)) return false;
            var ch = Characteristics[(int)CharacteristicsEnum.Rx_TeCommand_enum];
            if (ch == null) return false;
            GattCommunicationStatus result = GattCommunicationStatus.ProtocolError;
            try
            {
                result = await ch.WriteClientCharacteristicConfigurationDescriptorAsync(notifyType);
                if (!NotifyRx_ValueChanged_Set)
                {
                    // Only set the event callback once
                    NotifyRx_ValueChanged_Set = true;
                    ch.ValueChanged += NotifyRxCallback;
                }

            }
            catch (Exception e)
            {
                Status.ReportStatus($"NotifyRx: {e.Message}", result);
                return false;
            }
            Status.ReportStatus($"NotifyRx: set notification", result);

            return true;
        }

        private void NotifyRxCallback(GattCharacteristic sender, GattValueChangedEventArgs args) 
        {
            var datameaning = "BYTES|HEX|Rx";
            var parseResult = BluetoothDeviceController.BleEditor.ValueParser.Parse(args.CharacteristicValue, datameaning);
            Rx = parseResult.ValueList.GetValue("Rx").AsString;

            RxEvent?.Invoke(parseResult);

        }

        public void NotifyRxRemoveCharacteristicCallback() 
        {
            var ch = Characteristics[(int)CharacteristicsEnum.Rx_TeCommand_enum];
            if (ch == null) return;
            NotifyRx_ValueChanged_Set = false;
            ch.ValueChanged -= NotifyRxCallback;
        }
        ...
        private bool _Tx_set = false;
        public string Tx
        {
            get { return _Tx; }
            internal set { if (_Tx_set && value == _Tx) return; _Tx = value; _Tx_set = true; OnPropertyChanged(); }
        }
        ...
        /// <param name="cacheMode">Caching mode. Often for data we want uncached data.</param>
        /// <returns>BCValueList of results; each result is named based on the name in the characteristic string. E.G. U8|Hex|Red will be named Red</returns>
        public async Task<BCBasic.BCValueList> ReadTx(BluetoothCacheMode cacheMode = BluetoothCacheMode.Uncached)
        {
            if (!await EnsureCharacteristicAsync(CharacteristicsEnum.Tx_TeCommand_enum)) return null;
            IBuffer result = await ReadAsync(CharacteristicsEnum.Tx_TeCommand_enum, "Tx", cacheMode);
            if (result == null) return null;

            var datameaning = "BYTES|HEX|Tx";
            var parseResult = BluetoothDeviceController.BleEditor.ValueParser.Parse(result, datameaning);
            Tx = parseResult.ValueList.GetValue("Tx").AsString;
        ...
        /// <returns></returns>
        public async Task WriteTx(byte[] Tx)
        {
            if (!await EnsureCharacteristicAsync(CharacteristicsEnum.Tx_TeCommand_enum)) return;
            ...
            {
                await WriteCommandAsync(CharacteristicsEnum.Tx_TeCommand_enum, "Tx", command, GattWriteOption.WriteWithoutResponse);
            }
            else for (int i=0; i<command.Length; i+= MAXBYTES)
            {
                ...
                await WriteCommandAsync(CharacteristicsEnum.Tx_TeCommand_enum, "Tx", subcommand, GattWriteOption.WriteWithoutResponse);
            }
        }
