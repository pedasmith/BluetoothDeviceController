...
namespace BluetoothProtocols
{
    ...
    /// This class was automatically generated 2025-02-15::10:14
    /// </summary>
...
// method.list for Result
        // DBG: NOTE: METHOD+PROPERTY for ListOutput=parent Source=Services/Characteristics/ReadProperties
        ...
        {
            var ensureResult = await EnsureCharacteristicAsync(CharacteristicsEnum.Result_Car_enum);
            if (ensureResult != GattCommunicationStatus.Success) 
            {
                return false;
            }

            var ch = Characteristics[(int)CharacteristicsEnum.Result_Car_enum];
        ...
        private void NotifyResultCallback(GattCharacteristic sender, GattValueChangedEventArgs args) 
        {
        ...
        public void NotifyResultRemoveCharacteristicCallback() 
        {
            var ch = Characteristics[(int)CharacteristicsEnum.Result_Car_enum];
            if (ch == null) return;
            NotifyResult_ValueChanged_Set = false;
            ch.ValueChanged -= NotifyResultCallback;
        }
...
// method.list for Command
        ...
        /// <returns></returns>
        public async Task<GattCommunicationStatus> WriteCommand(String Command)
        {
            var ensureResult = await EnsureCharacteristicAsync(CharacteristicsEnum.Command_Car_enum);
            if (ensureResult != GattCommunicationStatus.Success) 
            {
                return ensureResult;
            }
            ...
            var retval = await WriteCommandAsync(CharacteristicsEnum.Command_Car_enum, "Command", command, GattWriteOption.WriteWithoutResponse);
            ...
            //{
            //    await WriteCommandAsync(CharacteristicsEnum.Command_Car_enum, "Command", command, GattWriteOption.WriteWithoutResponse);
            //}
            ...
            //    await WriteCommandAsync(CharacteristicsEnum.Command_Car_enum, "Command", subcommand, GattWriteOption.WriteWithoutResponse);
            //}
            return retval;
        }
