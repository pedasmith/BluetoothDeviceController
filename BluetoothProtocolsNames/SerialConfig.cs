...
// The SerialConfig will be updated later

namespace BluetoothProtocolsNames
{

    public static class VariableSetUtilities
    {
        public static void Init(this VariableSet curr, Dictionary<string, VariableDescription> values)
        {
            foreach (var (name, newValue) in values)
            {
                if (newValue.CurrValueIsString)
                {
                    curr.CurrStringValues[name] = newValue.InitString;
                }
                else
                {
                    curr.CurrValues[name] = newValue.Init;
                }
            }
        }
        public static void SetCurrValue(this VariableSet curr, Dictionary<string, VariableDescription> values)
        {
            foreach (var (name, newValue) in values)
            {
                if (newValue.CurrValueIsString)
                {
                    curr.CurrStringValues[name] = newValue.CurrValueString;
                }
                else
                {
                    curr.CurrValues[name] = newValue.CurrValue;
                }
            }
        }
    }
    ...
    public class Command
    {
        ...
        public string DoCompute()
        {
            ...
            {
                var calculateResult = IotNumberFormats.ValueCalculate.Calculate(strcommand, double.NaN, null, Variables);
